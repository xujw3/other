 ...
 * [in=quan] 识别英文全称
 * https://raw.githubusercontent.com/xujw3/other/refs/heads/main/rename3.js#flag&test&timeout=5000
 ...
 * 节点测试参数
 * [test]   测试节点可用性并删除无效节点
 * [timeout=3000] 设置测试超时时间，单位毫秒，默认3000ms
 */
...
const nx = inArg.nx || false,
  ...
  nm = inArg.nm || false,
  test = inArg.test || false,
  timeout = inArg.timeout ? parseInt(inArg.timeout) : 3000;
...
const rurekey = {
  ...
  "Korea Chuncheon": /Chuncheon|Seoul|ICN/g,
  "Hong Kong": /Hongkong|HONG KONG|HKG/gi,
  "United Kingdom London": /London|Great Britain|EDI|LHR|MAN/g,
  "Dubai United Arab Emirates": /United Arab Emirates|DXB/g,
  "Taiwan TW 台湾 🇹🇼": /(台|Tai\s?wan|TW|KHH|TPE).*?🇨🇳|🇨🇳.*?(台|Tai\s?wan|TW)/g,
  "United States": /USA|Los Angeles|San Jose|Silicon Valley|Michigan|ANC|ATL|AUS|BNA|BOS|BUF|CLT|CMH|DEN|DFW|DTW|EWR|FSD|HNL|IAD|IAH|IND|JAX|LAS|LAX|MCI|MEM|MFE|MGM|MIA|MSP|OKC|OMA|ORD|PDX|PHL|PHX|PIT|RDU|RIC|SAN|SEA|SFO|SJC|SLC|SMF|STL|TLH|TPA/g,
  澳大利亚: /澳洲|墨尔本|悉尼|土澳|(深|沪|呼|京|广|杭)澳|ADL|BNE|CBR|HBA|MEL|PER|SYD/g,
  德国: /(深|沪|呼|京|广|杭)德(?!.*(I|线))|法兰克福|滬德|DUS|FRA|HAM|MUC|STR|TXL/g,
  香港: /(深|沪|呼|京|广|杭)港(?!.*(I|线))/g,
  日本: /(深|沪|呼|京|广|杭|中|辽)日(?!.*(I|线))|东京|大坂|FUK|KIX|NRT|OKA/g,
  新加坡: /狮城|(深|沪|呼|京|广|杭)新|SIN/g,
  美国: /(深|沪|呼|京|广|杭)美|波特兰|芝加哥|哥伦布|纽约|硅谷|俄勒冈|西雅图|芝加哥|CT|CU|CM|电信|移动|联通|官方|优选|💦/g,
  波斯尼亚和黑塞哥维那: /波黑共和国/g,
  印尼: /印度尼西亚|雅加达/g,
  印度: /孟买/g,
  阿联酋: /迪拜|阿拉伯联合酋长国/g,
...
async function testProxy(proxy) {
  return new Promise((resolve) => {
    let testUrl;
    // 根据代理类型选择测试URL
    if (proxy.type === "ss" || proxy.type === "ssr" || proxy.type === "trojan" || 
        proxy.type === "vmess" || proxy.type === "vless") {
    ...
    try {
      ...
      timeoutTimer = setTimeout(() => {
        resolve({ proxy, available: false, delay: timeout });
      }, timeout);
      
      $httpClient.get(testRequest, (error, response, data) => {
        clearTimeout(timeoutTimer);
        const delay = Date.now() - startTime;
        
        if (error || !response || response.status !== 200) {
          resolve({ proxy, available: false, delay });
        } else {
          resolve({ proxy, available: true, delay });
        }
      });
    } catch (e) {
      clearTimeout(timeoutTimer);
...
async function testProxies(proxies) {
  console.log(`开始测试 ${proxies.length} 个节点...`);
  const testResults = await Promise.all(proxies.map(proxy => testProxy(proxy)));
  
  const availableProxies = testResults
    .filter(result => result.available)
    .map(result => {
      // 可以选择在节点名称中添加延迟信息
      if (result.delay < 1000) {
        result.proxy.name = `${result.proxy.name} [${result.delay}ms]`;
      } else {
        result.proxy.name = `${result.proxy.name} [${(result.delay/1000).toFixed(1)}s]`;
      }
      return result.proxy;
    });
  
  console.log(`测试完成，${availableProxies.length} 个节点可用，${proxies.length - availableProxies.length} 个节点不可用`);
